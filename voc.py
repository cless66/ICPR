...
def readlabeltxt(txtpath, height, width, hbb=True):
    ...
    with open(txtpath, 'r') as f_in:  # 打开txt文件
        ...
        for i, splitline in enumerate(splitlines):
            ...
            # 如果是hbb,hbb形式:xmin，ymin，xmax，ymax
            if hbb:
                xx1 = min(x1, x2, x3, x4)
                xx2 = max(x1, x2, x3, x4)
                yy1 = min(y1, y2, y3, y4)
                ...
                boxes.append(box)
            else:  # 否则是obb,obb形式x0，y0，x1，y1，x2，y2，x3，y3
...
if __name__ == '__main__':
    data_path = r'C:\Users\rina\Desktop\深度学习\网络图像的文本检测'
    images_path = os.path.join(data_path, 'image_train_0')  # 样本图片路径
    labeltxt_path = os.path.join(data_path, 'txt_train')  # DOTA标签的所在路径
    anno_new_path = os.path.join(data_path, 'ICPR_xml')  # 新的voc格式存储位置（hbb形式）
    ext = '.jpg'  # 样本图片的后缀
    dirs = os.listdir(r"C:\Users\rina\Desktop\深度学习\网络图像的文本检测\txt_train")
    for dir_ in dirs:
        filenames = os.listdir(labeltxt_path+'\\'+dir_)  # 获取每一个txt的名称
        for filename in filenames:
            print(filename)
            filepath = labeltxt_path + '/' +dir_+'/'+ filename  # 每一个DOTA标签的具体路径
            print(filepath)
            picname = os.path.splitext(filename)[0] + ext
            pic_path = os.path.join(images_path+'/'+dir_, picname)
        ...
        # 书写xml
            xml_path=anno_new_path + '/' + dir_
            isexistlabelxml = os.path.exists(xml_path)
            if not isexistlabelxml:
                os.makedirs(xml_path)
            writeXml(xml_path, picname, W, H, D, boxes, hbb=True)
